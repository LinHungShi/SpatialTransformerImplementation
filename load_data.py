"""
This code contains implementation of several utility functions that load the required datasets
Some code is based on examples from http://deeplearning.net/tutorial/gettingstarted.html
"""
...
########################################################
####### TODO: convert from CIFAR-10 to SVHN ############
########################################################
def load_svhn(theano_shared=True):
    ...
    for i in range(4):
        ...
        train_set['data']=np.concatenate((train_set['data'],temp['data']/255.),axis=0)
        train_set['labels']=np.concatenate((train_set['labels'].flatten(),temp['labels'].flatten()),axis=0)
